...
app.use((req, res, next) => {
    const logDirTask27 = path.join(__dirname, "logs");
...
function readLogTask21(fileName) {
    try {
        const filePath = path.join(__dirname, "runtime_logs", fileName);
...
const runtimeLogDirTask25 = path.join(__dirname, "runtime_logs");
...
// =====================================================
// TASK 35 DATABASE CONNECTION (Mongo)
// =====================================================

const connectDBTask35Server = async () => {
    try {
        const mongoUriTask35 =
            process.env.MONGO_URI ||
            "mongodb://127.0.0.1:27017/uccis";

        await mongoose.connect(mongoUriTask35);
...
connectDBTask35Server();

// =====================================================
// TASK 36 DATABASE CONNECTION (Mongo)
// NOTE: Task 36's own server.js called `connectDB()` unconditionally at
// startup, same as Task 35's file did with its own connect function. This
// is now the FOURTH Mongo-connect invocation point across this codebase
// (Task 26's connectDB, config/db.js's connectDBTask35, this file's
// connectDBTask35Server above, and now connectDBTask36 below) all aimed at
// the same "uccis" database. Calling mongoose.connect() again while
// already connected is not necessarily a hard error in recent Mongoose
// versions, but it's redundant and worth consolidating — kept here as
// Task 36 wrote it (invoked directly, fire-and-forget, no .catch of its
// own since connectDBTask36 already exits the process on failure).
// =====================================================

connectDBTask36();

// =====================================================
// TASK 37 DATABASE CONNECTION (Mongo)
// NOTE: Task 37's own file used the same MONGO_URI fallback and did its
// own `mongoose.connect(...)` inside a `connectDatabase()` function, then
// awaited it from a `startServer()` function before calling
// `server.listen(...)`. That's now a FIFTH Mongo-connect invocation point
// aimed at the same "uccis" database (see the Task 36 note above for the
// first four). Kept here, fire-and-forget, exactly as redundant as Task
// 36's own was — but its `startServer()`/`server.listen(...)` wrapper is
// NOT kept: this file already creates its shared `http` server and calls
// `server.listen(PORT, ...)` unconditionally further down (Task 15's
// original code). Calling `server.listen()` a second time on top of that
// would attempt to bind the same port twice and crash the process, the
// same problem already called out in the Task 36 note for its own
// `app.listen`. So Task 37's connect logic is preserved as a standalone
// function, but it no longer gates or duplicates the actual listen call.
// =====================================================

const connectDatabaseTask37 = async () => {
    try {
        const mongoUriTask37 =
            process.env.MONGO_URI ||
            "mongodb://127.0.0.1:27017/uccis";

        await mongoose.connect(mongoUriTask37);
...
// Task 38: WebSocket server (new — using ws library for raw WebSocket)
