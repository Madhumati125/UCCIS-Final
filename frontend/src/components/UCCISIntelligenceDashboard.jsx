...
const UCCISIntelligenceDashboard = () => {
  ...
  // Function to determine zone card class
  const getZoneCardClass = (zone, index, risk) => {
    // Check if this zone falls into a special-case color group
    const zoneLabel = zone.zone_name || `Zone ${index + 1}`;
    const redZones = ["Zone 1", "Zone 4", "Zone 7"];
    const orangeZones = ["Zone 2", "Zone 5", "Zone 8"];
    const blueZones = ["Zone 3", "Zone 6"];

    if (redZones.includes(zoneLabel)) {
      return "red";
    }

    if (orangeZones.includes(zoneLabel)) {
      return "orange";
    }

    if (blueZones.includes(zoneLabel)) {
      return "blue";
    }

    // Otherwise use risk-based classification
    if (risk >= 75) return "high";
    if (risk >= 40) return "medium";
    return "low";
  };

  // Provides a fallback reason when the API doesn't send one,
  // based on risk score and prediction level.
  const getZoneReason = (zone, risk) => {
    if (zone.reason && zone.reason.trim() !== "") {
      return zone.reason;
    }

    const prediction = (zone.prediction || "LOW").toUpperCase();

    if (risk >= 75 || prediction === "HIGH") {
      return "High anomaly score triggered elevated risk assessment.";
    }

    if (risk >= 40 || prediction === "MEDIUM") {
      return "Moderate deviation detected in recent zone activity.";
    }

    return "No significant anomalies detected in this zone.";
  };
        ...
        <div className="zone-grid">
          {zones.length === 0 ? (
            ...
            zones.map((zone, index) => {
              const risk = Number(zone.risk_score);
              const cardClass = getZoneCardClass(zone, index, risk);
